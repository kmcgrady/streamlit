...
clean:
	...
	rm -rf frontend/app/test_results
	rm -f frontend/app/src/autogen/proto.js
	rm -f frontend/app/src/autogen/proto.d.ts
	rm -rf frontend/app/public/reports
...
ifndef CIRCLECI
	...
	mkdir -p frontend/app/src/autogen
...
.PHONY: react-build
react-build:
	cd frontend/app/ ; yarn run build
...
frontend-fast:
	cd frontend/app/ ; yarn run buildFast
	rsync -av --delete --delete-excluded --exclude=reports \
		frontend/app/build/ lib/streamlit/static/
...
# Run JS unit tests.
jstest:
	cd frontend/app; TESTPATH=$(TESTPATH) yarn run test

.PHONY: jscoverage
# Run JS unit tests and generate a coverage report.
jscoverage:
	cd frontend/app; yarn run test --coverage --watchAll=false
...
notices:
	...
	./scripts/append_license.sh frontend/app/src/vendor/twemoji-LICENSE.txt
	./scripts/append_license.sh frontend/app/src/vendor/Segment-LICENSE.txt
	./scripts/append_license.sh frontend/app/src/vendor/react-bootstrap-LICENSE.txt
